...
void create_bmp(RGB *bitmap, int height, int width, const char *filename) {
  ...
  image.file_header.filesz = 2 * sizeof(char) + sizeof(bmpfile_header_t) +
                             sizeof(bmp_dib_header_t) +
                             height * width * sizeof(RGB);
  image.file_header.creator1 = image.file_header.creator2 = 0;
  image.file_header.bmp_offset =
      2 * sizeof(char) + sizeof(bmpfile_header_t) + sizeof(bmp_dib_header_t);
  ...
  fwrite(&(image.file_header), sizeof(char), sizeof(bmpfile_header_t),
         out_file);
